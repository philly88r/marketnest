...
/**
 * Search for competitors for a given keyword using Selenium with Bing
 */
router.post('/search', async (req, res) => {
  ...
  try {
    console.log(`Starting DIRECT Selenium search for keyword: ${keyword}`);
    ...
    const competitors = results.slice(0, 3);
    
    // Return only the real search results we found - NEVER use placeholders
    res.json({ competitors });
  } catch (error) {
    console.error('Error searching for competitors with Selenium:', error);
